...
export function consistencyPass(plan: Plan): { plan: Plan; report: ConsistencyReport } {
  ...
  if (plan.timeline.phases.length < plan.protocol.steps.length) {
    // Soft warning — phases legitimately group multiple protocol steps.
    warnings.push(
      `timeline has ${plan.timeline.phases.length} phases but protocol has ${plan.protocol.steps.length} steps — phases may group steps`
    );
  }
